...
Hypergol provides stubs for Tensorflow/Torch models and :class:`BaseBatchProcessor` abstraction to connect the model at training and deployment to the data model (and datasets). To enable iterative development and `SOLID <https://en.wikipedia.org/wiki/SOLID>`_ style development an opinionated abstraction is provided through :class:`BaseTensorflowModel` derived from `keras.Model` and :class:`BaseTensorflowModelBlock` derived from `keras.layers.Layer`. For Torch :class:`BaseTorchModel` and :class:`BaseTorchModelBlock` is derived from `torch.nn.Module`. By following the proposed structure, Hypergol provides :class:`TensorflowModelManager` and :class:`TorchModelManager` that handles training and evaluation, saving the model and any metrics for tensorboard. The models are packaged with the correct signature derived from the `get_outputs` function that enables deployment as well. The generated scripts ensure that full end-to-end data lineage is provided to enable complete output transparency from the output (in the client) back to the training source data and the actual version of code that run at each points.
